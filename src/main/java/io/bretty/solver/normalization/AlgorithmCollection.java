...
public class AlgorithmCollection {
   ...
   */
  public static Set<FunctionalDependency> check3NF(Set<Attribute> attrs, Set<FunctionalDependency> fds) {
    Set<Set<Attribute>> keys = computeCandidateKeys(attrs, fds);
    Set<Attribute> primes = new HashSet<>();
    for (Set<Attribute> k : keys) {
      primes.addAll(k);
    }

    Set<FunctionalDependency> violating = new HashSet<>();
   ...
   */
  public static Set<FunctionalDependency> checkBCNF(Set<Attribute> attrs, Set<FunctionalDependency> fds) {
    Set<Set<Attribute>> keys = computeCandidateKeys(attrs, fds);
    Set<FunctionalDependency> violating = new HashSet<>();
    for (FunctionalDependency fd : fds) {
      boolean contains = false;
   ...
   */
  public static Set<FunctionalDependency> checkLossyDecomposition(Set<Attribute> attrs, Set<FunctionalDependency> fds,
      Set<Set<Attribute>> subattrs) {
    Set<FunctionalDependency> lost = new HashSet<>();
    Set<FunctionalDependency> decomposed = new HashSet<>();
   ...
   */
  public static Set<Attribute> closure(Set<Attribute> attrs, Set<FunctionalDependency> fds) {
   ...
   * @param fds a set of FD's
   */
  public static void combineRight(Set<FunctionalDependency> fds) {
    ...
    for (Set<Attribute> left : map.keySet()) {
      fds.add(new FunctionalDependency.Builder().left(left).right(map.get(left)).build());
    }
  }
   ...
   */
  public static boolean equivalent(Set<FunctionalDependency> a, Set<FunctionalDependency> b) {
    Set<Attribute> names = new HashSet<>();
    for (FunctionalDependency fd : a) {
      names.addAll(fd.getLeft());
      names.addAll(fd.getRight());
    }
    for (FunctionalDependency fd : b) {
      names.addAll(fd.getLeft());
      names.addAll(fd.getRight());
    }
   ...
   */
  public static Set<Set<Attribute>> computeSuperKeys(Set<Attribute> attrs, Set<FunctionalDependency> fds) {
   ...
   */
  public static Set<Set<Attribute>> computeCandidateKeys(Set<Attribute> attrs, Set<FunctionalDependency> fds) {
    Set<Set<Attribute>> superkeys = computeSuperKeys(attrs, fds);
   ...
   */
  public static Set<FunctionalDependency> minimalBasis(Set<FunctionalDependency> fds) {
   ...
   */
  public static Set<FunctionalDependency> projection(Set<Attribute> attrs, Set<FunctionalDependency> fds) {
    ...
    for (Set<Attribute> sa : powerset) {
      ...
      result.add(new FunctionalDependency.Builder().left(sa).right(closure).build());
    }
    // return result;
   ...
   * @param fds a set of FD's
   */
  public static void removeTrivial(Set<FunctionalDependency> fds) {
    Set<FunctionalDependency> toRemove = new HashSet<>();
    Set<FunctionalDependency> toAdd = new HashSet<>();
    for (FunctionalDependency fd : fds) {
      if (fd.left.containsAll(fd.right)) {
        ...
        if (!toRemoveFromRight.isEmpty()) {
          ...
          toRemove.add(fd);
          toAdd.add(new FunctionalDependency.Builder().left(fd.left).right(right).build());
        }
      }
    }
    fds.addAll(toAdd);
   ...
   */
  public static int removeUnnecessaryEntireFD(Set<FunctionalDependency> fds) {
    int count = 0;
    while (true) {
      FunctionalDependency toRemove = null;
      boolean found = false;
      for (FunctionalDependency fd : fds) {
        Set<FunctionalDependency> remaining = new HashSet<>(fds);
        remaining.remove(fd);
   ...
   */
  public static int removeUnnecessaryLeftSide(Set<FunctionalDependency> fds) {
    int count = 0;
    while (true) {
      boolean found = false;
      FunctionalDependency toRemove = null;
      FunctionalDependency toAdd = null;
      int loop = 0;
      for (FunctionalDependency fd : fds) {
        Set<Attribute> left = fd.getLeft();
        Set<Attribute> right = fd.getRight();
        if (left.size() > 1) {
          for (Attribute a : left) {
            ...
            remaining.remove(a);
            Set<FunctionalDependency> alternative = new HashSet<>(fds);
            alternative.remove(fd);
            toAdd = new FunctionalDependency.Builder().left(remaining).right(right).build();
   ...
   * @param fds a set of FD's
   */
  public static void splitRight(Set<FunctionalDependency> fds) {
    Set<FunctionalDependency> toRemove = new HashSet<>();
    Set<FunctionalDependency> toAdd = new HashSet<>();
    for (FunctionalDependency fd : fds) {
      // Set<Attribute> right = fd.getRight();
      if (fd.right.size() > 1) {
        // Set<Attribute> left = fd.getLeft();
        for (Attribute a : fd.right) {
          toAdd.add(new FunctionalDependency.Builder().left(fd.left).right(a).build());
        }
        toRemove.add(fd);
      }
    }
