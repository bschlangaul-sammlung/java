...
public class Canvas {
  ...
  public Canvas(final int x, final int y) {
    this(x, y, 10, "Canvas", new Color[] { Color.RED, Color.GREEN, Color.BLUE, Color.YELLOW, Color.GRAY, Color.PINK,
        Color.CYAN, Color.DARK_GRAY, Color.MAGENTA, Color.ORANGE, Color.LIGHT_GRAY });
  }
   ...
   * @param x       Number of colored areas in x-direction
   * @param y       Number of colored areas in y-direction
   * @param pSize   Side length of a colored area in pixels
   * @param title   Window title
   * @param palette The palette of used colors
   */
  public Canvas(final int x, final int y, final double pSize, final String title, final Color[] palette) {
    this(x, y, pSize, title, new Color[x][y], palette, x * y);
  }

  private Canvas(final int x, final int y, final double pSize, final String title, final Color[][] array,
      final Color[] palette, final int counter) {
    ...
    try {
      javax.swing.SwingUtilities.invokeAndWait(new Runnable() {
        public void run() {
            ...
            protected void paintComponent(Graphics g) {
              ...
              for (int i = 0; i < x; i++) {
                for (int j = 0; j < y; j++) {
                  ...
                  // paint area
                  g.fillPolygon(
                      new int[] { (int) (i * xFac), (int) (i * xFac), (int) ((i + 1) * xFac), (int) ((i + 1) * xFac) },
                      new int[] { (int) (j * yFac), (int) ((j + 1) * yFac), (int) ((j + 1) * yFac), (int) (j * yFac) },
                      4);
                  if (unset) {
                    // paint cross
                    g.setColor(Color.BLACK);
                    g.fillPolygon(
                        new int[] { (int) (i * xFac), (int) (i * xFac), (int) ((i + 1) * xFac),
                            (int) ((i + 1) * xFac) },
                        new int[] { (int) ((j + 1) * yFac), (int) (j * yFac), (int) ((j + 1) * yFac),
                            (int) (j * yFac) },
                        4);
                  }
                }
          ...
          colorPanel.setPreferredSize(new Dimension((int) (pSize * x), (int) (pSize * y)));
  ...
  /**
   * @return <code>true</code>, if the number of colorize calls is equal or
   *         greater as the number of points
   */
   ...
   *
   * @param array The pixels of the window
